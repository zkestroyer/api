...
async function registerWebhook() {
    logEvent('POST', 'Registering Receiver (Port 3005) for webhooks...');
    try {
        ...
        if (response.ok) {
            logEvent('SUCCESS', 'Receiver successfully connected!');
            alert('Webhook receiver successfully connected! Open http://localhost:3005 in another tab to see events arrive.');
        } else {
...
form.addEventListener('submit', async (e) => {
    ...
    logEvent('POST', `Adding student & triggering webhook...`);
...
async function deleteStudent(id) {
    ...
    logEvent('DELETE', `Deleting student & triggering webhook...`);
