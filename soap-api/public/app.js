...
async function fetchStudents() {
    logEvent('GET', 'Fetching via SOAP Bridge...');
    try {
        ...
        if (response.ok) {
            ...
            logEvent('SUCCESS', `Fetched ${students.length} students via SOAP`);
...
form.addEventListener('submit', async (e) => {
    ...
    logEvent('POST', `Adding student via SOAP Bridge...`);
    ...
    try {
        ...
        if (response.ok) {
            ...
            logEvent('SUCCESS', `Added student via SOAP (ID: ${addedStudent.id.substring(0, 8)}...)`);
...
async function deleteStudent(id) {
    ...
    logEvent('DELETE', `Deleting student via SOAP Bridge...`);
    ...
    try {
        ...
        if (response.ok) {
            logEvent('SUCCESS', 'Student deleted via SOAP');
